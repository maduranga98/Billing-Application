...
class _AddOutletState extends State<AddOutlet> with TickerProviderStateMixin {
    ...
    try {
      ...
      if (widget.outlet != null) {
        ...
        final updates = <String, dynamic>{
          ...
          // ADDED: Include route information in updates
        ...
        // UPDATED: Add new outlet with route information
        final outletData = {
          'outletName': _outletNameController.text.trim(),
          'address': _addressController.text.trim(),
          'phoneNumber': _phoneController.text.trim(),
        ...
        success = await outletProvider.addOutlet(
          ...
          // ADDED: Pass route information to provider
          routeId: widget.routeId,
          routeName: widget.routeName,
        );
        ...
        if (success) {
          _showSnackBar(
            outletProvider.isConnected
                ? 'Outlet added successfully to ${widget.routeName}!'
                : 'Outlet saved offline for ${widget.routeName}. Will sync when online.',
          );
        }
      }
  ...
  // FIXED: Replace syncOfflineOutlets method with sync pending outlets
  Future<void> _syncOfflineOutlets() async {
    ...
    try {
      _showSnackBar('Syncing offline outlets...');

      // Call the sync method from outlet service directly
      await OutletService.syncPendingOutlets(authProvider.currentSession!);

      // Refresh the outlet list to show updated sync status
      await outletProvider.refreshOutlets(authProvider.currentSession!);

      _showSnackBar('Outlets synced successfully!');
    } catch (e) {
      _showSnackBar('Failed to sync outlets: $e', isError: true);
    }
  }

  // FIXED: Update _buildSyncButton to show correct count
          ...
          child: OutlinedButton(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  outletProvider.offlineOutletCount > 0
